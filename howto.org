...
*** first do it the "correct" way, using custom/customize for defcustom variables
*** benchmark that against a naive setq blanket strategy
*** if setq is significantly faster, consider using it selectively for variables that don't leverage :set or :initialize
